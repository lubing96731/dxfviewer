...
class Polyline : public Entity {
 private:
  ...
  std::vector<std::vector<PointF>> m_polyline;

 public:
  Polyline(std::vector<Point *> &vertexes, uint m = 0, uint n = 0, int flag = 0,
 ...
 public:
  ...
  const std::vector<std::vector<PointF>> &GetPolyline() const {
    return m_polyline;
  }
