...
class Spline : public Entity {
 public:
  Spline(uint degree, uint nKnots, uint nControl, uint nFit, int flags,
 ...
 public:
  ...
  const std::vector<Point *> &GetDrawablePoints() const {
    return m_drawabelPoints;
  }
 ...
 private:
  ...
  void ExportSpline();
  void CalXYMinMax(std::vector<Point *> &controlPts, double &xMin, double &xMax,
